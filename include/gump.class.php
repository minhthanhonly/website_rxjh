...
class GUMP
{
	...
	*/
	...
	protected function validate_alpha_dash($field, $input, $param = NULL)
	{
		...
		if(!preg_match("/^([a-z0-9ÀÁÂẬẪẮẶÈÉỀỆÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêếìíîïðòóôõöùúûüýÿ_-])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
	...
	protected function validate_alpha_space($field, $input, $param = NULL)
	{
		...
		if(!preg_match("/^([a-z0-9ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝàáâãäåçèéêếìíîïðòóôõöùúûüýÿ\s])+$/i", $input[$field]) !== FALSE)
		{
			return array(
				'field' => $field,
				'value' => $input[$field],
